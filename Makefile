...
nat_compare: init
	...
	test  "$(shell kubectl run --pod-running-timeout=10m -it --generator=run-pod/v1 curl --image appropriate/curl -- curl ifconfig.co)" == "$(shell gcloud compute addresses describe default-primary-cluster-regional-nat-nat-external-address --region $(google_region) --project $(google_project) |grep 'address:'| awk '{print $$2}')"
