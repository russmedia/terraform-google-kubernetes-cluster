...
nat_compare: init
	...
	test  "$(shell kubectl run -it --generator=run-pod/v1 curl --image appropriate/curl -- curl ifconfig.co 2>/dev/null)" == "$(shell gcloud compute addresses describe default-primary-cluster-regional-nat-nat-external-address --region $(google_region) --project $(google_project) |grep 'address:'| awk '{print $$2}')"
